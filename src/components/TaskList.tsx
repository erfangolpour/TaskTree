...
export const TaskList: React.FC = () => {
  ...
  const rootTasks = filteredAndSortedTasks.filter(task => 
    task.parentIds.length === 0 || 
    !task.parentIds.some(parentId => 
      filteredAndSortedTasks.some(t => t.id === parentId)
    )
  );
        ...
        <SortableContext items={rootTasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
          {rootTasks.map(task => (
            <TaskItem 
              key={task.id} 
              task={task} 
              allTasks={filteredAndSortedTasks}
            />
          ))}
        </SortableContext>
      </div>
